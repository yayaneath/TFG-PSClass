...
features    = size(data1_n, 2);
resultsAcc  = zeros(28, 13);
line = 1;
...
for i = 1 : features - 1
   for j = i + 1 : features
       resultsAux = zeros(tests, 13);
       
       for it = 1 : tests
           message      = sprintf('* %d * [1-2] Dimensiones %d - %d *', ...
            it, i, j);
           disp(message);

           %%% Extract features data
           data1        = [data1_n(:, i) data1_n(:, j)];
           data2        = [data2_n(:, i) data2_n(:, j)];
           ...
           results      = [i j parts1 parts2 accuracy tn fp fn tp ...
               recall precision foldError trainTime];
           resultsAux(it, :) = results;

           message      = sprintf('[1-2] Dimensiones %d - %d !OK!', i, j);
...
resultsFile  = sprintf('[1-2] dim%d_v3', dimensions);
...
for i = 1 : features - 1
   for j = i + 1 : features
       ...
       for it = 1 : tests
           message      = sprintf('* %d * [1-3] Dimensiones %d - %d *', ...
            it, i, j);
           disp(message);

           %%% Extract features data
           data1        = [data1_n(:, i) data1_n(:, j)];
           data3        = [data3_n(:, i) data3_n(:, j)];
           ...
           message      = sprintf('[1-3] Dimensiones %d - %d !OK!', i, j);
...
resultsFile  = sprintf('[1-3] dim%d_v3', dimensions);
csvwrite([resCSVs resultsFile '.csv'], resultsAcc);
save([resMats resultsFile '.mat'], 'resultsAcc');
...
for i = 1 : features - 1
   for j = i + 1 : features
       ...
       for it = 1 : tests
           message      = sprintf('* %d * [2-3] Dimensiones %d - %d *', ...
            it, i, j);
           disp(message);
           ...
           message      = sprintf('[2-3] Dimensiones %d - %d !OK!', i, j);
...
resultsFile  = sprintf('[2-3] dim%d_v3', dimensions);
csvwrite([resCSVs resultsFile '.csv'], resultsAcc);
save([resMats resultsFile '.mat'], 'resultsAcc');